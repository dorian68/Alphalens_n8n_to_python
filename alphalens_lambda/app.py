...
class MacroCommentary(BaseModel):
    ExecutiveSummary: str = Field(
        description=(
            ...
            "and the dominant narrative driving markets. Typically 2–4 concise sentences, no bullet points."
        )
    )
...
class MarketCommentaryAnchor(BaseModel):
    summary: str = Field(
        description="2–3 sentence institutional synthesis of macro and directional bias."
    )
...
FX / Commodities / Crypto → "XXX/USD" (e.g. XAU/USD, EUR/USD, BTC/USD)
...
horizons: List[int] ⚠️ CRITICAL

Forecast horizons expressed as NUMBER OF FUTURE CANDLES.

✅ Must be positive integers

❌ Strings like "12h", "3d" are FORBIDDEN

Timeframe → Horizons rules (MANDATORY)
...
Convert durations → integer candle counts

Never send strings in horizons

If unsure → do not call the tool
...
"horizons": [3, 6] ✅
    ...
    try:
        ...
        # 🔒 catch-all (never let the graph crash)
        return {
            "status": "error",
